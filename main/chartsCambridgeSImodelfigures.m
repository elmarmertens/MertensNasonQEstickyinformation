...
Nparticles = 1e5; % only used for scaling ESS
...
for m = 1 : 4
    for doInflationNoise = [true false]
        ...
        switch v
            ...
            case 2 % CPI
                xtickdates = datenum(1981:5:2018,1,1);
            otherwise
                xtickdates = datenum(1968:5:2018,1,1);
        end
        ...
        figname = strcat('figRELESS-', modellabel);
        figure
        ...
        lowndx = ESS < .01;
        ess = NaN(size(ESS));
        ess(lowndx) = ESS(lowndx);
        plot(dates, ess, 'rd', 'linewidth', 4, 'markersize', 10)
        ylim([0 1])
        nbershades(dates)
        ...
        set(gca, 'fontsize', fontsize)
        hold on
        h1 = plot(dates, GAPNOISEhatSI, 'k-', 'linewidth', 1);
        h2 = plot(dates, GAPNOISEhatRE, 'r--', 'linewidth', 1);
        if v == 1
            ylim([-4 14])
        else
            ylim([-6 8])
