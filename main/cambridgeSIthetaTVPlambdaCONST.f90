PROGRAM main
  ...
  CHARACTER (LEN=200) :: modellabel ='SIthetaTVPlambdaCONST'
  ...
  CALL plefilter(doSecondResamplingStep, dofAPFfattail, doInflationNoise, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWpihat, DRAWpifcst, DRAWxhat, DRAWsqrtSigmaX, DRAWxsig, DRAWxgain, Nx, NsigmaX, Nw, Ex0, sqrtVx0, DRAWgapnoisehat, p, DRAWa, a0, a0V, DRAWsiga, sigaT, sigaDof, DRAWlambda, lambdaAlpha, lambdaBeta, lambdaN, DRAWsvol, Nsv, Eh0, Vh0, DRAWhInno, hvarT, hvarDof, DRAWsigma, sigmaT, sigmaDof, VSLstream,timer)
  ...
  IF (doSmoother) THEN
     ...
     ! ! SMOOTHER
     ! ! ----------------------------------------------------------------------------
     ALLOCATE (SMOOTHERsvol(Nsv,Nsmoother,0:T), SMOOTHERlambda(Nsmoother), SMOOTHERa(Nsmoother,0:T))
     ALLOCATE (SMOOTHERsigma(Ny,Nsmoother))
     ALLOCATE (SMOOTHERhinno(Nsv,Nsmoother))

     call hrulefill
     print *, 'STARTING w/SMOOTHER ...'
     CALL plesmoother(doInflationNoise, Nsmoother, smootherNparticles, T, p, SMOOTHERa, SMOOTHERlambda, Nsv, SMOOTHERsvol, SMOOTHERhinno, SMOOTHERsigma, Nparticles, PARAMweights, PARAMsiga, PARAMlambda, PARAMhInno, PARAMsigma, a0, a0V, Eh0, Vh0, Ex0, sqrtVx0, Ny, y, yNaN, Nx, NsigmaX, Nw, VSLstream)
     ...
     if (doLinearUncertainty) then
        ...
        forall (j=0:T) STATElambda(j)         = sum(SMOOTHERlambda) / Nsmoother
        ...
        SIndx    = .True.
        SIndx(1) = .False.
        call linearstateVarianceFilter(horizons, SIndx, measurementErrorVol, size(horizons), Xsig, T, Nx, Nw, sqrtVx0, p, STATEa, STATElambda, STATEsvol, Nsv)
...
SUBROUTINE plefilter(doSecondResamplingStep, dofAPFfattail, doInflationNoise, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWpihat, DRAWpifcst, DRAWxhat, DRAWsqrtSigmaX, DRAWxsig, DRAWxgain, Nx, NsigmaX, Nw, Ex0, sqrtVx00, DRAWgapnoisehat, p, DRAWa, a0, a0V, DRAWsiga, sigaT, sigaDof, DRAWlambda, lambdaAlpha0, lambdaBeta0, lambdaN0, DRAWsvol, Nsv, Eh0, Vh0, DRAWhInno, hvarT, hvarDof, DRAWsigma, sigmaT, sigmaDof, VSLstream, timer)
  ...
  use cambridgebox, only : drawdeltatruncnorm, drawbetas
  use vslbox
  use omp_lib
  use timerbox
  ...
  DO j=1,T
     ...
     !$OMP PARALLEL DO SHARED(xposterior, xsig, vecSqrtSigmaX, lambda, adrift, SVol, DRAWsigma, llf, Nparticles, Nsv, j, y, yNaN, Ny, Nx, Nw, Nynonan, Nsurveys, p, ndxTrendRE, ndxGapRE, ndxTrendSI, ndxGapSI, ndxGapREstart, ndxGapREstop, ndxGapSIstart, ndxGapSIstop, shockndxTrend, shockndxGap) SHARED(dofAPFfattail) FIRSTPRIVATE(A,B,C,qrLWORK) PRIVATE(ytilde, sqrtR, Bsv, logdetSigmaY, errcode, xprior, SigmaX, sqrtSigmaX, sqrtSigmaY, qrR, TID) DEFAULT(NONE) SCHEDULE(STATIC)
...
SUBROUTINE particlefilter(doSecondResamplingStep, dofAPFfattail, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWxhat, DRAWsqrtSigmaX, Nx, NsigmaX, Nw, Ex0, sqrtVx00, p, DRAWa, a0, a0V, siga, lambda, DRAWh, Nsv, Eh0, Vh0, hInno, sigma, VSLstream)
  ...
  INTENT(INOUT) :: PARTICLEweights, logMDD, DRAWa, DRAWxhat, DRAWsqrtSigmaX, DRAWh, VSLstream !, timer
  ...
  DO k=1,Nparticles
     ...
     if (abs(adrift(k)) > 1.0d-4 .AND. lambda > 1.0d-4) then 
        gaptransition                  = 0.0d0
        gaptransition(1:p,1:p)         = adrift(k)
        gaptransition(p+1:2*p,1:p)     = (1 - lambda) * adrift(k)
        gaptransition(p+1:2*p,p+1:2*p) = lambda * adrift(k)
  ...
  DO j=1,T
     ...
     DO k = 1,Nparticles
        ...
        A(ndxgapSIstart:ndxgapSIstop,ndxgapREstart:ndxgapREstop) = (1 - lambda) * adrift(k)
        A(ndxgapSIstart:ndxgapSIstop,ndxgapSIstart:ndxgapSIstop) = lambda * adrift(k)
...
SUBROUTINE plesmoother(doInflationNoise, Nsmoother, Nparticles, T, p, SMOOTHERa, SMOOTHERlambda, Nsv, SMOOTHERh, SMOOTHERhinno, SMOOTHERsigma, Nparamparticles, PARAMweights, PARAMsiga, PARAMlambda, PARAMhInno, PARAMsigma, a0, a0V, Eh0, Vh0, Ex0, sqrtVx0, Ny, y, yNaN, Nx, NsigmaX, Nw, VSLstream)
  ...
  !$OMP& FIRSTPRIVATE(C), &
  !$OMP& PRIVATE(invR, invSqrtR, ytilde, Bsv, w, cdf, hshock, z, ashock, aCDFlb, aCDFub, SVpdf, Apdf, eta, sqrtSigmaX, logdetsqrtLAMBDA, sqrtLAMBDA, sqrtOmegaHAt, lambdaHat, lambda, zhat, prevSV, prevH, prevA, A, qr1R, qr2R, qr3R), &
  !$OMP& PRIVATE(logMDD, PARTICLEweights, FILTERxhat, FILTERsqrtSigmaX, FILTERa, FILTERh), &
  ...
  DO k=1,Nsmoother
     ...
     call particlefilter(doSecondResamplingStep, dofAPFfattail, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, FILTERxhat, FILTERsqrtSigmaX, Nx, NsigmaX, Nw, Ex0, sqrtVx0, p, FILTERa, a0, a0V, SMOOTHERsiga(k), SMOOTHERlambda(k), FILTERh, Nsv, Eh0, Vh0, SMOOTHERhInno(:,k), SMOOTHERsigma(:,k), VSLlocal)
     ...
     DO j=T-1,0,-1
        ...
        A(ndxgapSIstart:ndxgapSIstop,ndxgapREstart:ndxgapREstop) = (1 -  SMOOTHERlambda(k)) * prevA
        A(ndxgapSIstart:ndxgapSIstop,ndxgapSIstart:ndxgapSIstop) =  SMOOTHERlambda(k) * prevA
	...
	! try computing w in logs as much as possible
        w = -0.5d0 * (SVpdf  + Apdf + eta) - logdetsqrtLAMBDA - log(aCDFub - aCDFlb) + log(PARTICLEweights(:,j))
...
SUBROUTINE particleSmootherX(T, Ny, y, yNaN, Nsv, Nsmoother, SMOOTHERa, p, SMOOTHERlambda, SMOOTHERsvol, NsmootherX, SMOOTHERx, Nx, Nw, Ex0, sqrtVx00, SMOOTHERsigma)
  ...
  DO k=1,Nsmoother
     ...
     forall (j=1:T) A(ndxgapSIstart:ndxgapSIstop,ndxgapREstart:ndxgapREstop,j) = (1 - SMOOTHERlambda(k)) * SMOOTHERa(k,j)
     forall (j=1:T) A(ndxgapSIstart:ndxgapSIstop,ndxgapSIstart:ndxgapSIstop,j) = SMOOTHERlambda(k)* SMOOTHERa(k,j)
     ...
     gaptransition(p+1:2*p,1:p)     = (1 - SMOOTHERlambda(k)) * SMOOTHERa(k,0)
     gaptransition(p+1:2*p,p+1:2*p) = SMOOTHERlambda(k) * SMOOTHERa(k,0)
...
SUBROUTINE linearstateVarianceFilter(horizons, SIndx, measurementErrorVol, Ny, Xsig, T, Nx, Nw, sqrtVx0, p, STATEa, STATElambda, STATEsvol, Nsv)
  ...
  INTEGER :: J, I, T, Nx, Ny, Nsv, p, Nw, Nsurveys
