PROGRAM main
  ...
  CHARACTER (LEN=200) :: modellabel ='SIthetaTVPlambdaTVPnormcdf'
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)     :: DRAWxlambda
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:)       :: PARAMxlambda_xbar, PARAMxlambda_sigma
  ...
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)   :: DRAWsiga
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)   :: DRAWxlambda_xbar, DRAWxlambda_sigma ! _xbar and _sigma are mean and vol of xlambda
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:,:) :: DRAWsigma, DRAWhInno
  DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:,:)   :: SMOOTHERhinno ! legacy
  ...
  ! x-lambda: parameters
  DOUBLE PRECISION :: xlambda_rho
  DOUBLE PRECISION :: xlambda_xbar0, xlambda_xbarV0inv, xlambda_ssr0
  INTEGER :: xlambda_dof0
  ...
  ! Xlambda:
  ! normal gamma priors for AR1 parameters
  xlambda_rho                = 0.99d0
  xlambda_xbar0              = 0.0d0
  xlambda_xbarV0inv          = 1.0d0

  xlambda_dof0               = 3
  xlambda_ssr0               = 1.0d0 * dble(xlambda_dof0 - 2) ! prior over unconditional distribution of xlambda
  ...
  ALLOCATE (DRAWxlambda(Nparticles,0:T), DRAWxlambda_xbar(Nparticles,1:T), DRAWxlambda_sigma(Nparticles,1:T), STAT=status)
  IF (status /= 0) THEN
     WRITE (*,*) 'Allocation problem (DRAWxlambda)'
  END IF
  ...
  DRAWxlambda_xbar   = 0.0d0
  DRAWxlambda_sigma  = 0.0d0

  CALL plefilter(doSecondResamplingStep, dofAPFfattail, doInflationNoise, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWpihat, DRAWpifcst, DRAWpipersistence, DRAWxhat, DRAWsqrtSigmaX, DRAWxsig, DRAWxgain, Nx, NsigmaX, Nw, Ex0, sqrtVx0, DRAWgapnoisehat, p, DRAWa, a0, a0V, DRAWsiga, sigaT, sigaDof, DRAWlambda, DRAWxlambda, DRAWxlambda_xbar, DRAWxlambda_sigma, xlambda_xbar0,  xlambda_xbarV0inv, xlambda_ssr0, xlambda_dof0, xlambda_rho, DRAWsvol, Nsv, Eh0, Vh0, DRAWhInno, hvarT, hvarDof, DRAWsigma, sigmaT, sigmaDof, VSLstream,timer)
  ...
  END IF
  WRITE(4,'(a20,I40)') 'p: ', p
  WRITE(4,'(a20,F40.2)') 'xlambda_rho: ', xlambda_rho
  if (doInflationNoise) THEN
  ...
  ! X-LAMBDA
  filename  = 'XLAMBDAHAT' // filext
  theta1 = sum(PARTICLEweights(:,1:T) * DRAWxlambda(:,1:T), 1)
  call savevec(theta1, filename)
  WRITE (*,*) 'STORED XLAMBDAHAT'
  ! draw distribution
  filename  = 'XLAMBDA' // filext
  FORALL (j=1:T,k=1:Nmixturedraws) theta2(k,j) = DRAWxlambda(ndx(k,j),j) 
  CALL storeEstimatesTranspose(theta2,T,Nmixturedraws,filename)
  WRITE (*,*) 'STORED XLAMBDA'
  ...
  ! SCALE PARAMETERS
  ! siga
  filename  = 'SIGAHAT'  // filext
  theta1 = sum(PARTICLEweights(:,1:T) * (DRAWsiga(:,1:T) ** 2), 1)
  call savevec(theta1, filename)
  WRITE (*,*) 'STORED SIGAHAT'
  ! draw distribution
  filename  = 'SIGA'  // filext
  FORALL (j=1:T,k=1:Nmixturedraws) theta2(k,j) = DRAWsiga(ndx(k,j),j) ** 2 
  CALL storeEstimatesTranspose(theta2,T,Nmixturedraws,filename)
  WRITE (*,*) 'STORED SIGA'
  ...
  ! XLAMBDA PARAMETERS
  ! xlambda_mean
  filename  = 'XLAMBDA_XBARHAT' // filext
  theta1 = sum(PARTICLEweights(:,1:T) * DRAWxlambda_xbar(:,1:T))
  call savevec(theta1, filename)
  WRITE (*,*) 'STORED XLAMBDA_XBARHAT'
  ! draw distribution
  filename  = 'XLAMBDA_XBAR' // filext
  FORALL (j=1:T,k=1:Nmixturedraws) theta2(k,j) = DRAWxlambda_xbar(ndx(k,j),j) 
  CALL storeEstimatesTranspose(theta2,T,Nmixturedraws,filename)
  WRITE (*,*) 'STORED XLAMBDA_XBAR'


  ! sigma
  filename  = 'XLAMBDA_SIGMAHAT'  // filext
  theta1 = sum(PARTICLEweights(:,1:T) * DRAWxlambda_sigma(:,1:T), 1)
  call savevec(theta1, filename)
  WRITE (*,*) 'STORED XLAMBDA_SIGMAHAT'
  ! draw distribution
  filename  = 'XLAMBDA_SIGMA'  // filext
  FORALL (j=1:T,k=1:Nmixturedraws) theta2(k,j) = DRAWxlambda_sigma(ndx(k,j),j) 
  CALL storeEstimatesTranspose(theta2,T,Nmixturedraws,filename)
  WRITE (*,*) 'STORED XLAMBDA_SIGMA'
  ...
  ALLOCATE (PARAMxlambda_xbar(Nparticles), PARAMxlambda_sigma(Nparticles))
  ALLOCATE (PARAMsiga(Nparticles))

  PARAMweights =  PARTICLEweights(:,T)
  PARAMhInno   =  DRAWhInno(:,:,T)
  PARAMsigma   =  DRAWsigma(:,:,T)
  PARAMxlambda_xbar  = DRAWxlambda_xbar(:,T)
  PARAMxlambda_sigma = DRAWxlambda_sigma(:,T)
  PARAMsiga    = DRAWsiga(:,T)

  DEALLOCATE (PARTICLEweights)
  DEALLOCATE (DRAWhInno,DRAWsigma)
  DEALLOCATE (DRAWxlambda_xbar,DRAWxlambda_sigma)
  DEALLOCATE (DRAWsiga)
  ...
  IF (doSmoother) THEN
     ...
     CALL plesmoother(doInflationNoise, Nsmoother, smootherNparticles, T, p, SMOOTHERa, SMOOTHERlambda, Nsv, SMOOTHERsvol, SMOOTHERhinno, SMOOTHERsigma, Nparticles, PARAMweights, PARAMsiga, PARAMxlambda_xbar, PARAMxlambda_sigma, PARAMhInno, PARAMsigma, a0, a0V, xlambda_rho, Eh0, Vh0, Ex0, sqrtVx0, Ny, y, yNaN, Nx, NsigmaX, Nw, VSLstream)

     DEALLOCATE (PARAMweights)
     DEALLOCATE (PARAMsigma, PARAMhInno)     
     DEALLOCATE (PARAMxlambda_xbar, PARAMxlambda_sigma)
     ...
     do nn=1,Nsurveys
        do i = 1,nn
           FORALL (k=1:Nsmoother,j=1:T) FEVgap(k,j,nn) =  FEVgap(k,j,nn) + (SMOOTHERa(k,j) ** (2 * (nn - i))) * SMOOTHERsvol(mm,k,j) ** 2 * exp( 0.5d0 * (SMOOTHERhinno(mm,k) ** 2) *  dble(i))
        end do ! i
     end do  ! nn
     ...
     ! VE
     forall (k=1:Nsmoother,j=1:T,nn=1:Nsurveys) VE(k,j,nn) =  SMOOTHERsvol(1,k,j) ** 2  * exp(0.5d0 * (SMOOTHERhinno(1,k) ** 2)) + SMOOTHERa(k,j) ** (2 * (nn - 1)) * SMOOTHERsvol(2,k,j) ** 2  * exp(0.5d0 * (SMOOTHERhinno(2,k) ** 2))
     ! R2 
     ...
     forall (k=1:Nsmoother,j=1:T,nn=1:Nsurveys) EV(k,j,nn) = EVstates(ndxtrendRE,k,j) + SMOOTHERa(k,j) ** nn * EVstates(ndxgapRE,k,j)
     forall (k=1:Nsmoother,j=1:T,nn=1:Nsurveys) EVSI(k,j,nn) = EVstates(ndxtrendSI,k,j) + SMOOTHERa(k,j) ** nn * EVstates(ndxgapSI,k,j) ! note: bias w.r.t. *current* SI forecast
	...
	allocate (theta2(Nsmoother,0:T))
        ...
        filename = trim(this) // filext
        call savemat(SMOOTHERa(:,0:T),filename)
        WRITE (*,*) 'STORED ' // trim(this)
     ...
     filename  = 'smootherA' // filext
     CALL storeEstimatesTranspose(SMOOTHERa(:,1:T),T,Nsmoother,filename)
     WRITE (*,*) 'SMOOTHER: STORED A' 
     FORALL (j=1:T,k=1:Nsmoother) theta2(k,j) = SMOOTHERa(k,j) - SMOOTHERa(k,1) 
     filename  = 'smootherDELTAA' // filext
     CALL storeEstimatesTranspose(theta2,T,Nsmoother,filename)
     WRITE (*,*) 'SMOOTHER: STORED DELTAA' 

     filename  = 'smootherABSA' // filext
     FORALL (j=1:T,k=1:Nsmoother) theta2(k,j) = abs(SMOOTHERa(k,j))
     CALL storeEstimatesTranspose(theta2(:,1:T),T,Nsmoother,filename)
     WRITE (*,*) 'SMOOTHER: STORED ABSA'
     ...
     if (doLinearUncertainty) then
        ...
        STATEa       = sum(SMOOTHERa, 1) / Nsmoother
        ...
        deallocate (Xsig, STATEa, STATElambda, STATEsvol)
...
SUBROUTINE plefilter(doSecondResamplingStep, dofAPFfattail, doInflationNoise, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWpihat, DRAWpifcst, DRAWpipersistence, DRAWxhat, DRAWsqrtSigmaX, DRAWxsig, DRAWxgain, Nx, NsigmaX, Nw, Ex0, sqrtVx00, DRAWgapnoisehat, p, DRAWa, a0, a0V, DRAWsiga, sigaT, sigaDof, DRAWlambda, DRAWxlambda, DRAWxlambda_xbar, DRAWxlambda_sigma, xlambda_xbar0, xlambda_xbarV0inv, xlambda_ssr0, xlambda_dof0, xlambda_rho, DRAWsvol, Nsv, Eh0, Vh0, DRAWhInno, hvarT, hvarDof, DRAWsigma, sigmaT, sigmaDof, VSLstream, timer)

  USE embox, only : savemat, savevec, int2str, mean, hrulefill

  use gibbsbox, only : drawNDXsysresample, igammaDraws, normalgammaDrawsUnivariate
  ...
  INTENT(INOUT) :: PARTICLEweights, logMDD, DRAWa, DRAWlambda, DRAWxhat, DRAWsqrtSigmaX, DRAWxsig, DRAWxgain, DRAWsvol, DRAWsiga, DRAWhInno, DRAWsigma, VSLstream, timer
  INTENT(INOUT) :: DRAWxlambda
  INTENT(IN)    :: xlambda_xbar0, xlambda_xbarV0inv, xlambda_ssr0, xlambda_dof0, xlambda_rho
  INTENT(IN)    :: T,Ny,y,yNaN, Nx, NsigmaX, Nw, Ex0, sqrtVx00, Nsv,Eh0,Vh0, Nparticles, p, a0, a0V, sigaT, sigaDof, hvarT, hvarDof, sigmaT, sigmaDof
  INTENT(INOUT) :: DRAWpihat, DRAWpifcst
  INTENT(INOUT) :: DRAWpipersistence
  INTENT(INOUT) :: DRAWxlambda_xbar, DRAWxlambda_sigma 
  ...
  DOUBLE PRECISION, DIMENSION(Nparticles,0:T) :: DRAWxlambda
  ...
  DOUBLE PRECISION :: CholeskiGain(Nx,Ny)
  ...
  ! LAMBDA and XLAMABDA objects
  DOUBLE PRECISION :: lambda(Nparticles), xlambda(Nparticles), xlambdalag(Nparticles)
  ! xlambda-AR1 parameters
  DOUBLE PRECISION, DIMENSION(Nparticles,T)   :: DRAWxlambda_sigma
  DOUBLE PRECISION, DIMENSION(Nparticles,T)   :: DRAWxlambda_xbar
  DOUBLE PRECISION :: xlambda_rho, xlambda_interceptscale
  ! normal-gamma priors
  DOUBLE PRECISION :: xlambda_xbar0, xlambda_xbarV0inv, xlambda_ssr0
  INTEGER :: xlambda_dof0

  ! sufficient statistics
  DOUBLE PRECISION :: PREVxlambda_xbar(Nparticles), PREVxlambda_xbarVinv(Nparticles), PREVxlambda_ssr(Nparticles)
  INTEGER :: PREVxlambda_dof
  ! helper variables for NG-update
  DOUBLE PRECISION :: rhs(1,1), lhs(1,1)
  ! DOUBLE PRECISION, DIMENSION(1,1) :: xbar, xbarVinv
  ! DOUBLE PRECISION :: ssr
  DOUBLE PRECISION :: xbar0, xbarV0inv
  ...
  DOUBLE PRECISION :: xlambdaDELTA(Nparticles)
  ...
  rhs      = 1.0d0
  xlambda_interceptscale = (1.0d0 - xlambda_rho) / sqrt(1.0d0 - xlambda_rho ** 2)
  ...
  ! init sufficients statistics for xlambda-AR1 parameters
  forall(k=1:Nparticles) PREVxlambda_xbar(k)     = xlambda_xbar0
  forall(k=1:Nparticles) PREVxlambda_xbarVinv(k) = xlambda_xbarV0inv
  forall(k=1:Nparticles) PREVxlambda_ssr(k)      = xlambda_ssr0
  PREVxlambda_dof = xlambda_dof0
  ...
  ! Lambda(0)
  errcode = vdrnggaussian(VSLmethodGaussian, VSLstream, Nparticles, xlambdalag, 0.0d0, 1.0d0)
  ! NOTE: always drawing initial value from standard-normla (i.e. uniform prior over lambda0)
  xlambda    = xlambdalag
  ! make sure that initial lambda is *inside* unit circle (to avoid problems with initial variance for SI-GAP)
  where (xlambda > 4.0d0)  xlambda = 4.0d0   
  where (xlambda < -4.0d0) xlambda = -4.0d0   
  call vdcdfnorm(Nparticles, xlambda, lambda)
  ...
  !$OMP& SHARED(vecSqrtSigmaX, p, SVol, lambda, adrift, Ny, Nx, Nw, Nsv, Nparticles, sqrtVx00, ndxgap, shockndxGap), &
  !$OMP& PRIVATE(gaptransition,gapshock0loadings, sqrtVx0, ygap0sqrtvariance, errcode) 
  DO k=1,Nparticles

     sqrtVx0         = transpose(sqrtVx00)

     if (abs(adrift(k)) > 1.0d-4 .AND. lambda(k) > 1.0d-4 .AND. abs(adrift(k)) * lambda(k) > 1.0d-6) then 
        gaptransition                  = 0.0d0
  ...
  DO j=1,T
     ...
     !$OMP& SHARED(xposterior, xsig, vecSqrtSigmaX, lambda, adrift, SVol, DRAWsigma, llf, Nparticles, Nsv, j, y, yNaN, Ny, Nx, Nw, Nynonan, Nsurveys, p, ndxTrendRE, ndxGapRE, ndxTrendSI, ndxGapSI, ndxGapREstart, ndxGapREstop, ndxGapSIstart, ndxGapSIstop, shockndxTrend, shockndxGap), &
     !$OMP& SHARED(dofAPFfattail), &
     !$OMP& FIRSTPRIVATE(A,B,C,qrLWORK), &
     !$OMP& PRIVATE(ytilde, sqrtR, Bsv, logdetSigmaY, errcode, xprior, SigmaX, sqrtSigmaX, sqrtSigmaY, qrR, TID) 
     ...
     if (Nynonan > 0) then
        ...
        FORALL(k=1:Nparticles) shufflevec(k) = xlambda(ndx(k))
        xlambda = shufflevec
        ...
        ! xlambda-AR1 stats
        FORALL(k=1:Nparticles) shufflevec(k) = PREVxlambda_xbar(ndx(k))
        PREVxlambda_xbar = shufflevec
        FORALL(k=1:Nparticles) shufflevec(k) = PREVxlambda_xbarVinv(ndx(k))
        PREVxlambda_xbarVinv = shufflevec

        FORALL(k=1:Nparticles) shufflevec(k) = PREVxlambda_ssr(ndx(k))
        PREVxlambda_ssr = shufflevec
     ...
     ! draw AR1 parameters from NormalGamma
     call normalgammadrawsUnivariate(Nparticles, DRAWxlambda_xbar(:,j), DRAWxlambda_sigma(:,j), PREVxlambda_xbar, PREVxlambda_xbarVinv, PREVxlambda_ssr, PREVxlambda_dof, VSLstream)
     ...
     xlambdalag  = xlambda
     errcode      = vdrnggaussian(VSLmethodGaussian, VSLstream, Nparticles, xlambdaDELTA, 0.0d0, 1.0d0)
     forall (k=1:Nparticles) xlambdaDELTA(k) = sqrt(1.0d0 - xlambda_rho ** 2) * DRAWxlambda_sigma(k,j) * xlambdaDELTA(k)
     forall (k=1:Nparticles) xlambda(k) = (1.0d0 - xlambda_rho) * DRAWxlambda_xbar(k,j) + xlambda_rho * xlambdalag(k) + xlambdaDELTA(k)
     where (xlambda >  4.0d0) xlambda =  4.0d0   
     where (xlambda < -4.0d0) xlambda = -4.0d0   
     call vdcdfnorm(Nparticles, xlambda, lambda)
     ...
     !$OMP PARALLEL DO SHARED(xposterior, xsig, vecSqrtSigmaX, lambda, adrift, SVol, Kgain, DRAWsigma, llf, Nparticles, Nsv, j, y, yNaN, Ny, Nx, Nw, Nynonan, Nsurveys, p, ndxTrendRE, ndxGapRE, ndxTrendSI, ndxGapSI, ndxGapREstart, ndxGapREstop, ndxGapSIstart, ndxGapSIstop, shockndxTrend, shockndxGap) SHARED(pihat,pifcst,pipersistence) PRIVATE(CholeskiGain) FIRSTPRIVATE(A,B,C,qrLWORK) PRIVATE(ytilde, sqrtR, Bsv, logdetSigmaY, errcode, xprior, SigmaX, sqrtSigmaX, sqrtSigmaY, qrR, TID) DEFAULT(NONE) SCHEDULE(STATIC)
     ...
     DO k = 1,Nparticles
        ...
        ! INFLATION FORECASTS
        ! ------------------------------------------------------------------------
        forall(i=1:Nsurveys) pihat(i,k) = xposterior(ndxTrendRE,k) + (adrift(k) ** i) * xposterior(ndxGapRE,k)
        forall(i=1:Nsurveys) pifcst(i,k) = xposterior(ndxTrendSI,k) + (adrift(k) ** i) * xposterior(ndxGapSI,k)
     ...
     if (.NOT. yNaN(1,j)) then
        ! RE noise
        FORALL(k=1:Nparticles) DRAWnoise(k,j) = y(1,j) - xposterior(ndxTrendRE,k) - xposterior(ndxGapRE,k) 
        ! RE: i=1
        FORALL(k=1:Nparticles) DRAWgapnoisehat(k,j,1) = y(1,j) - xposterior(ndxTrendRE,k) 
        ! SI: i=2
        FORALL(k=1:Nparticles) DRAWgapnoisehat(k,j,2) = xposterior(ndxGapSI,k) + (1.0d0 - lambda(k)) * DRAWnoise(k,j)
     end if
     ...
     if (Nynonan > 0) then ! nothing to propagate if there was no observed data
        ...
        ! update xlambda-AR1 statistics
        PREVxlambda_dof  = PREVxlambda_dof + 1
        do k=1,Nparticles

           ! rescale xbar prior
           PREVxlambda_xbar(k)      = PREVxlambda_xbar(k) * xlambda_interceptscale
           PREVxlambda_xbarVinv(k)  = PREVxlambda_xbarVinv(k) / (xlambda_interceptscale ** 2)
           ! lhs
	   lhs = (xlambda(k) - xlambda_rho * xlambdalag(k)) / sqrt(1.0d0 - xlambda_rho ** 2)

    ! store priors
	   xbar0     = PREVxlambda_xbar(k)
           xbarV0inv = PREVxlambda_xbarVinv(k)
	   ! normal gamma update
           PREVxlambda_xbarVinv(k) =  xbarV0inv + 1.0d0
           PREVxlambda_xbar(k)     =  (xbarV0inv * xbar0 + lhs(1,1)) / PREVxlambda_xbarVinv(k) 
           PREVxlambda_ssr(k) = PREVxlambda_ssr(k) + (lhs(1,1) - PREVxlambda_xbar(k)) ** 2 + ((PREVxlambda_xbar(k) - xbar0) ** 2) * xbarV0inv

           ! rescale xbar posteriors
           PREVxlambda_xbar(k)     = PREVxlambda_xbar(k) /  xlambda_interceptscale
           PREVxlambda_xbarVinv(k) = PREVxlambda_xbarVinv(k) * (xlambda_interceptscale ** 2)
        ...
        if (doSecondResamplingStep) then
           ...
           FORALL(k=1:Nparticles) shufflevec(k) = xlambda(ndx(k))
           xlambda = shufflevec
           FORALL(k=1:Nparticles) shufflevec(k) = xlambdalag(ndx(k))
           xlambdalag = shufflevec
...
SUBROUTINE particlefilter(doSecondResamplingStep, dofAPFfattail, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, DRAWxhat, DRAWsqrtSigmaX, Nx, NsigmaX, Nw, Ex0, sqrtVx00, p, DRAWa, a0, a0V, siga, DRAWlambda, DRAWxlambda, xlambda_xbar, xlambda_sigma, xlambda_rho, DRAWh, Nsv, Eh0, Vh0, hInno, sigma, VSLstream)
  ...
  INTENT(IN)    :: T,Ny,y,yNaN, Nx, NsigmaX, Nw, Ex0, sqrtVx00, Nsv,Eh0,Vh0, Nparticles, p, a0, a0V
  ...
  DOUBLE PRECISION, INTENT(IN)                 :: xlambda_xbar, xlambda_sigma, xlambda_rho
  ...
  ! LAMBDA and XLAMABDA objects
  DOUBLE PRECISION :: lambda(Nparticles), xlambda(Nparticles), xlambdalag(Nparticles), xlambdaDELTA(Nparticles)
  DOUBLE PRECISION :: adrift(Nparticles), aPREV(Nparticles),  a0, a0V
  ...
  ! Lambda(0)
  errcode = vdrnggaussian(VSLmethodGaussian, VSLstream, Nparticles, xlambda, 0.0d0, 1.0d0)
  ! NOTE: always drawing initial value from standard-normal (i.e. uniform prior over lambda0)
  ! make sure that initial lambda is *inside* unit circle (to avoid problems with initial variance for SI-GAP)
  where (xlambda > 4.0d0)  xlambda = 4.0d0   
  where (xlambda < -4.0d0) xlambda = -4.0d0   
  call vdcdfnorm(Nparticles, xlambda, lambda)
  ...
  DO j=1,T
     ...
     xlambdalag  = xlambda
     errcode     = vdrnggaussian(VSLmethodGaussian, VSLstream, Nparticles, xlambdaDELTA, 0.0d0, 1.0d0)
     forall (k=1:Nparticles) xlambdaDELTA(k) = sqrt(1.0d0 - xlambda_rho ** 2) * xlambda_sigma * xlambdaDELTA(k)
     forall (k=1:Nparticles) xlambda(k) = (1.0d0 - xlambda_rho) * xlambda_xbar + xlambda_rho * xlambdalag(k) + xlambdaDELTA(k)
     where (xlambda >  4.0d0) xlambda =  4.0d0   
     where (xlambda < -4.0d0) xlambda = -4.0d0   
     call vdcdfnorm(Nparticles, xlambda, lambda)
...
SUBROUTINE plesmoother(doInflationNoise, Nsmoother, Nparticles, T, p, SMOOTHERa, SMOOTHERlambda, Nsv, SMOOTHERh, SMOOTHERhinno, SMOOTHERsigma, Nparamparticles, PARAMweights, PARAMsiga, PARAMxlambda_xbar, PARAMxlambda_sigma, PARAMhInno, PARAMsigma, a0, a0V, xlambda_rho, Eh0, Vh0, Ex0, sqrtVx0, Ny, y, yNaN, Nx, NsigmaX, Nw, VSLstream)
  ...
  DOUBLE PRECISION :: SMOOTHERsiga(Nsmoother) 
  DOUBLE PRECISION :: SMOOTHERxlambda_xbar(Nsmoother), SMOOTHERxlambda_sigma(Nsmoother) 
  ...
  DOUBLE PRECISION, INTENT(IN), DIMENSION(Nparamparticles)         :: PARAMxlambda_xbar, PARAMxlambda_sigma
  DOUBLE PRECISION, INTENT(IN), DIMENSION(Nparamparticles, Ny)     :: PARAMsigma

  DOUBLE PRECISION, INTENT(IN) :: a0, a0V, Eh0(Nsv), Vh0(Nsv), Ex0(Nx), sqrtVx0(Nx,Nx)
  DOUBLE PRECISION :: xlambda_rho
  ...
  DOUBLE PRECISION, DIMENSION(Nparticles,0:T)     :: FILTERxlambda
  ...
  double precision :: prevXLAMBDA
  double precision :: prevA

  ! filter particles and lindsten helpers
  DOUBLE PRECISION, DIMENSION(Nparticles)     :: w, SVpdf, xLAMBDApdf, eta, logdetsqrtLAMBDA
  ...
  DOUBLE PRECISION, DIMENSION(Nparticles)     :: xlambdashock
  ...
  forall (k=1:Nsmoother) SMOOTHERxlambda_sigma(k)  = PARAMxlambda_sigma(ndx(k))
  forall (k=1:Nsmoother) SMOOTHERxlambda_xbar(k)   = PARAMxlambda_xbar(ndx(k))
  ...
  !$OMP& SHARED(SMOOTHERsiga, SMOOTHERhinno,SMOOTHERsigma), &
  !$OMP& SHARED(SMOOTHERxlambda_xbar, SMOOTHERxlambda_sigma), &
  !$OMP& SHARED(SMOOTHERa, SMOOTHERlambda, SMOOTHERh), &
  !$OMP& PRIVATE(VSLlocal,TID), & 
  !$OMP& PRIVATE(j, i, errcode), & 
  !$OMP& FIRSTPRIVATE(C), &
  !$OMP& PRIVATE(invR, invSqrtR, ytilde, Bsv, w, cdf, hshock, z, xlambdashock, ashock, aCDFlb, aCDFub, SVpdf, Apdf, XLAMBDApdf, eta, sqrtSigmaX, logdetsqrtLAMBDA, sqrtLAMBDA, sqrtOmegaHAt, lambdaHat, lambda, zhat, prevSV, prevH, prevLAMBDA, prevXLAMBDA, prevA, A, qr1R, qr2R, qr3R), &
  !$OMP& PRIVATE(logMDD, PARTICLEweights, FILTERxhat, FILTERsqrtSigmaX, FILTERlambda, FILTERxlambda, FILTERa, FILTERh), &
  !$OMP& SHARED(a0, a0V, Eh0, Vh0, Ex0, sqrtVx0), &
  !$OMP& SHARED(xlambda_rho)
  ...
  DO k=1,Nsmoother
     ...
     call particlefilter(doSecondResamplingStep, dofAPFfattail, T, Ny, y, yNaN, Nparticles, PARTICLEweights, logMDD, FILTERxhat, FILTERsqrtSigmaX, Nx, NsigmaX, Nw, Ex0, sqrtVx0, p, FILTERa, a0, a0V, SMOOTHERsiga(k), FILTERlambda, FILTERxlambda, SMOOTHERxlambda_xbar(k), SMOOTHERxlambda_sigma(k), xlambda_rho, FILTERh, Nsv, Eh0, Vh0,  SMOOTHERhInno(:,k), SMOOTHERsigma(:,k), VSLlocal)
     ...
     prevXLAMBDA= FILTERxlambda(filterndx,T)
     ...
     DO j=T-1,0,-1
        ...
        ! - pdf of xLAMBDA move
        ! note: backwards AR1 regression has same moments as forward as long as AR(1) is stable
        xlambdashock = (FILTERxlambda(:,j) - xlambda_rho * prevXLAMBDA - (1.0d0 - xlambda_rho) * SMOOTHERxlambda_xbar) / SMOOTHERxlambda_sigma
        xLAMBDApdf   = xlambdashock ** 2
	...
	! try computing w in logs as much as possible
        w = -0.5d0 * (SVpdf + xLAMBDApdf  + Apdf + eta) - logdetsqrtLAMBDA - log(aCDFub - aCDFlb) + log(PARTICLEweights(:,j))
...
SUBROUTINE particleSmootherX(T, Ny, y, yNaN, Nsv, Nsmoother, SMOOTHERa, p, SMOOTHERlambda, SMOOTHERsvol, NsmootherX, SMOOTHERx, Nx, Nw, Ex0, sqrtVx00, SMOOTHERsigma)
  ...
  DO k=1,Nsmoother
     ...
     if (abs(SMOOTHERa(k,0)) > 1.0d-4 .AND. SMOOTHERlambda(k,0) > 1.0d-4) then 
...
SUBROUTINE linearstateVarianceFilter(horizons, SIndx, measurementErrorVol, Ny, Xsig, FcstPersistence, T, Nsurveys, Nx, Nw, sqrtVx0, p, STATEa, STATElambda, STATEsvol, Nsv)
  ...
  if (abs(STATEa(j)) > 1.0d-4 .AND. STATElambda(j) > 1.0d-4 .AND. abs(STATEa(j)) * STATElambda(j) > 1.0d-6) then
     gaptransition                  = 0.0d0
     ...
     CALL DLYAPsqrt(ygap0sqrtvariance, gaptransition, gapshock0loadings, 2 * p, Nw, errcode) 
     if (errcode /= 0) then
        write (*,*) 'DLYAP error (ygap0sqrtvariance)', errcode
        stop 1
     end if
  ...
  DO j=1,T
     ...
     ! call savemat(qrR, 'qrR.debug')
     ! call savemat(Mstar, 'Mstar.debug')
     ! stop 11
