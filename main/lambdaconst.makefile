THIS ?= cambridgeSIthetaTVPlambdaCONST # or: cambridgeSIthetaTVPlambdaCONST
...
$(THIS)stderr  : $(THIS)stderr.f90 $(toolboxes)
	$(FC) $(THIS)stderr.f90 -o $(THIS)stderr $(toolboxes) 


cambridgebox.o  : cambridgebox.f90 embox.o vslbox.o
	$(FC) -c cambridgebox.f90
...
compile	: $(THIS) $(THIS)stderr 

run	: $(THIS)
	rm -f *.debug
	time -p ./$(THIS) $(doInflationNoise) $(Nparticles) $(doSmoother) $(Nsmoother) $(smootherNparticles) $(DATALABEL) $(DATALABEL)  $(doSecondResamplingStep) $(dofAPFfattail) $(lambdaN)

runstderr : $(THIS)stderr 
	rm -f *.debug
	time -p ./$(THIS)stderr $(Nparticles) $(Ngrid) $(doInflationNoise) $(DATALABEL) $(doSecondResamplingStep) $(lambdaN)
